...
type DetailedArguments = YargsParser.DetailedArguments
type Configuration = YargsParser.Configuration
let [<Import("*","yargs")>] yargs: Argv = jsNative

type BuilderCallback<'T, 'R> =
    U3<(Argv<'T> -> Promise<Argv<'R>>), (Argv<'T> -> Argv<'R>), (Argv<'T> -> unit)>
...
type [<AllowNullLiteral>] Argv<'T> =
    [<Emit "$0($1...)">] abstract Invoke: unit -> U2<obj, Promise<obj>>
    [<Emit "$0($1...)">] abstract Invoke: args: string[] * ?cwd: string -> Argv<'T>
    ...
    abstract argv: U2<Arguments<'T>, Promise<Arguments<'T>>> with get, set
    ...
    abstract array: key: U2<'K, ResizeArray<'K>> -> Argv<'U>
    ...
    abstract boolean: key: U2<'K, ResizeArray<'K>> -> Argv<'U>
    ...
    abstract choices: key: 'K * values: 'C -> Argv<'U>
    abstract choices: choices: 'C -> Argv<'U>
    ...
    abstract coerce: key: U2<'K, ResizeArray<'K>> * func: (obj option -> 'V) -> Argv<'U>
    abstract coerce: opts: 'O -> Argv<'U>
    ...
    abstract command: command: U2<string, ResizeArray<string>> * description: string * ?builder: BuilderCallback<'T, 'U> * ?handler: (Arguments<'U> -> unit) * ?middlewares: ResizeArray<MiddlewareFunction> * ?deprecated: U2<bool, string> -> Argv<'U>
    abstract command: command: U2<string, ResizeArray<string>> * description: string * ``module``: CommandModule<'T, 'U> -> Argv<'U>
    abstract command: command: U2<string, ResizeArray<string>> * showInHelp: obj * ?builder: BuilderCallback<'T, 'U> * ?handler: (Arguments<'U> -> unit) * ?middlewares: ResizeArray<MiddlewareFunction> * ?deprecated: U2<bool, string> -> Argv<'T>
    ...
    abstract count: key: U2<'K, ResizeArray<'K>> -> Argv<'U>
    ...
    abstract ``default``: key: 'K * value: 'V * ?description: string -> Argv<'U>
    abstract ``default``: defaults: 'D * ?description: string -> Argv<'U>
    ...
    abstract demandOption: key: string * ?msg: string -> Argv<'T>
    abstract demandOption: key: string * ?demand: bool -> Argv<'T>
    abstract demandOption: key: ResizeArray<string> * ?demand: bool -> Argv<'T>
    ...
    abstract demandCommand: min: float -> Argv<'T>
    ...
    abstract deprecateOption: option: string * ?msg: string -> Argv<'T>
    ...
    abstract fail: func: U2<(string -> Error -> Argv<'T> -> obj option), bool> -> Argv<'T>
    ...
    abstract normalize: key: U2<'K, ResizeArray<'K>> -> Argv<'U>
    ...
    abstract number: key: U2<'K, ResizeArray<'K>> -> Argv<'U>
    /// <summary>Method to execute when a command finishes successfully.</summary>
    /// <param name="func">Is called with the successful result of the command that finished.</param>
    abstract onFinishCommand: func: (obj option -> unit) -> Argv<'T>
    ...
    abstract option: key: 'K * options: 'O -> Argv<'U>
    abstract option: options: 'O -> Argv<'U>
    /// This method can be used to make yargs aware of options that could exist.
    /// You can also pass an opt object which can hold further customization, like `.alias()`, `.demandOption()` etc. for that option.
    abstract options: key: 'K * options: 'O -> Argv<'U>
    abstract options: options: 'O -> Argv<'U>
    ...
    abstract parse: unit -> U2<obj, Promise<obj>>
    abstract parse: arg: U2<string, ResizeArray<string>> * ?context: obj * ?parseCallback: ParseCallback<'T> -> U2<obj, Promise<obj>>
    abstract parseSync: unit -> obj
    abstract parseSync: arg: U2<string, ResizeArray<string>> * ?context: obj * ?parseCallback: ParseCallback<'T> -> obj
    abstract parseAsync: unit -> Promise<obj>
    abstract parseAsync: arg: U2<string, ResizeArray<string>> * ?context: obj * ?parseCallback: ParseCallback<'T> -> Promise<obj>
    ...
    abstract positional: key: 'K * opt: 'O -> Argv<'U>
    ...
    /// <summary>Provide the usage data as a string.</summary>
    /// <param name="printCallback">a function with a single argument.</param>
    abstract showHelp: printCallback: (string -> unit) -> Argv<'T>
    ...
    /// will raise an error.
    abstract strictCommands: unit -> Argv<'T>
    abstract strictCommands: enabled: bool -> Argv<'T>
    ...
    /// will raise an error.
    abstract strictOptions: unit -> Argv<'T>
    abstract strictOptions: enabled: bool -> Argv<'T>
    ...
    abstract string: key: U2<'K, ResizeArray<'K>> -> Argv<'U>
...
type [<AllowNullLiteral>] Arguments<'T> =
    [<Emit "$0._">] abstract Arguments: U2<string, float>[]
    [<Emit "$0">] abstract Options: 'T
    [<Emit "$0[$1]">] abstract Item: key:string -> obj with get
...
type [<AllowNullLiteral>] Options =
    ...
    abstract deprecate: U2<bool, string> option with get, set
    /// boolean or string, mark the argument as deprecated, see `deprecateOption()`
    abstract deprecated: U2<bool, string> option with get, set
