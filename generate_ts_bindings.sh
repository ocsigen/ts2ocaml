...
node --stack-trace-limit=100000 ./output/ts2ocaml.js jsoo --int --stdlib node_modules/typescript/lib/lib.*.d.ts > ./output/ts2ocaml.mli
cp ./output/ts2ocaml.mli ./test/src/ts2ocaml.mli

function ts2ocaml () {
  node --stack-trace-limit=100000 ./output/ts2ocaml.js jsoo $1 > ./output/$2.mli
  if [ $? -ne 0 ]; then
    echo "failed: " $1
    rm output/$2.mli
    return
